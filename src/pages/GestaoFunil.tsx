...
const GestaoFunil = () => {
  ...
  const handleCreateCard = (listId: string, cardData: any) => {
    ...
    const updatedBoard = {
      ...currentBoard,
      lists: currentBoard.lists.map(list => list.id === listId ? {
        ...list,
  ...
  const filteredCards = currentBoard?.lists.flatMap(list => list.cards.filter(card => {
    ...
    const matchesTags = filter.tags.length === 0 || filter.tags.some(tagId => card.tags.includes(tagId));
    const matchesAssignee = !filter.assignee || card.assignees.includes(filter.assignee);
    const matchesPriority = !filter.priority || card.priority === filter.priority;
